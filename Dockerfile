...
RUN git config --global url."https://${GTK}:x-oauth-basic@github.com/".insteadOf "https://github.com/"

# 복사 및 빌드
COPY . .
RUN go mod tidy
RUN go mod download
RUN go build -ldflags="-s -w" -o podoroot ./application/cmd/main.go
...
COPY --from=builder /build/podoroot /
